...
| ~~**无 `ListWorkspaces`**~~ | **本条是错的，2026-08-09 勘误**。`workspace.ListWorkspaces` **一直存在且可用**（实测 `{page_num, page_size}` 返回完整 items）—— 它只是不在 qz CLI 的 spec 里。qzcli 目前仍从 `GetProjectForPage` 的 `space_list[]` 推导，能用但绕了远路 | ✅ 不是缺口。可直接改用 `ListWorkspaces` |
...
| ~~**v2 拿不到点券 / 预算**~~ | **本条是错的，2026-08-09 勘误**。原文说「169 个 action 里 budget/billing 零命中」—— 那是**在 qz CLI 的 spec 里**零命中，不是平台没有。实测 `project.GetProjectBudgetUsageOverview` 直接返回 `{total, used, remain, train, inference, storage}`，比 v1 的 `remain_budget` 还细；个人额度走 `project.GetProjectMemberBudgetUsage` | ✅ 不是缺口。唯一成立的部分：`GetProjectForPage` 自己不带预算值，要单独调 |
