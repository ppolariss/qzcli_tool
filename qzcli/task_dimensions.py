"""
基于 workspace ListTaskDimension 的任务维度查询与本地看板。

底层由 :mod:`qzcli.api` 统一按 v2 优先、仅路由不通时回落 v1 的策略调用。
"""
...
from .config import get_cookie, get_workspace_resources, load_all_resources
...
def _authenticated_cookie_context(api) -> Tuple[str, str]:
    ...
    cookie = api.ensure_authenticated(cookie_data.get("cookie", ""))
    return cookie, str(cookie_data.get("workspace_id", "") or "")
...
def _json_bytes(value: Any) -> bytes:
    """Encode standards-compliant JSON, rejecting any leaked NaN/Infinity."""
...
def _collect_group_node_names(
    ...
    page_nodes = _fetch_paginated_items(
        ...
        endpoint="/api/v2/workspace?Action=ListNodeDimension",
...
def _resolve_workspace_and_project(
    ...
    *,
    ...
    api = get_api()
    _, cookie_workspace_id = _authenticated_cookie_context(api)
    ...
    if not workspace_input and not allow_auto_workspace:
        workspace_id = cookie_workspace_id
        if not workspace_id:
        ...
        workspace_id, workspace_name = _pick_workspace(
            workspace_input,
            cookie_workspace_id if allow_auto_workspace else "",
            workspace_options,
        )
...
def _fetch_task_dimensions(
    ...
    *,
    ...
    api = get_api()
    cookie, _ = _authenticated_cookie_context(api)
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                for job_id in requested_job_ids:
                    try:
                        stopped = bool(api.stop_job(job_id))
                        results.append(
                            {"job_id": job_id, "stopped": stopped, "error": ""}
                        )
                    except Exception as exc:  # pragma: no cover - served path
                        results.append(
                            {
                ...
                body = _json_bytes({"results": results})
                self.send_response(200)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
