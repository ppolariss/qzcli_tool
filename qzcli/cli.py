...
from .create_commands import (
    cmd_batch as _legacy_cmd_batch,
    cmd_create as _legacy_cmd_create,
    cmd_create_hpc as _legacy_cmd_create_hpc,
)
from .resource_commands import (
    cmd_avail as _legacy_cmd_avail,
    cmd_resources as _legacy_cmd_resources,
    cmd_specs as _legacy_cmd_specs,
    cmd_workspaces as _legacy_cmd_workspaces,
)
from .resource_resolution import ResourceResolutionError, resolve_workspace_ref
from .store import JobRecord, get_store
from .task_dimensions import cmd_task_dimensions as _legacy_cmd_task_dimensions
from .user_jobs import (
    job_summary as _legacy_job_summary,
    list_user_jobs as _legacy_list_user_jobs,
    summarize_user_jobs as _legacy_summarize_user_jobs,
)
...
def cmd_user_jobs(args):
    ...
    jobs = jobs[:args.limit]
    counts = _legacy_summarize_user_jobs(result["jobs"])
    ...
    if args.output_json:
        print(json.dumps({
            ...
            **counts,
            ...
            "jobs": [_legacy_job_summary(job) for job in jobs],
        }, ensure_ascii=False, indent=2))
        return 0
...
def cmd_specs(args):
    """查询分区资源规格（本地 fork 兼容命令）。"""
    return _legacy_cmd_specs(args)


def cmd_task_dimensions(args):
    """查看 cluster_metric 任务维度（本地 fork 兼容命令）。"""
    return _legacy_cmd_task_dimensions(args)


def cmd_avail(args):
    """查询计算组空余节点，帮助决定任务应该提交到哪里"""
    if getattr(args, "cpu", False):
        return _legacy_cmd_avail(args)
...
def cmd_create(args):
    ...
    if payload["auto_fault_tolerance"]:
        payload["fault_tolerance_max_retry"] = getattr(
            args, "fault_tolerance_max_retry", 3
        )
...
def cmd_create_hpc(args):
    """本地 fork 兼容命令：通过 OpenAPI payload 提交 HPC 任务。"""
    return _legacy_cmd_create_hpc(args)
...
def main():
    ...
    workspaces_parser = subparsers.add_parser(
        "workspaces",
        aliases=["catalog", "lsws", "res", "resources"],
    ...
    # create-hpc 命令 - 创建 HPC 任务（本地 fork OpenAPI 兼容入口）
    create_hpc_parser = subparsers.add_parser(
        "create-hpc",
        aliases=["create-hpc-job"],
        help="创建并提交 HPC 任务到启智平台",
    )
    create_hpc_parser.add_argument("--name", "-n", required=True, help="任务名称")
    create_hpc_parser.add_argument(
        "--entrypoint",
        "--command",
        "-c",
        dest="entrypoint",
