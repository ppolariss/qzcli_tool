...
class Display:
    ...
    def print_logs(
        self, entries: List[Dict[str, Any]], raw: bool = False, json_mode: bool = False
    ):
    ...
    def print_jobs_table(
        ...
        for job in jobs:
            ...
            # 截短 job_id 便于显示
            short_job_id = (
                job.job_id[:20] + "..." if len(job.job_id) > 23 else job.job_id
            )
        ...
        if show_url:
            ...
            for job in jobs:
                if job.url:
                    ...
                    short_name = (
                        truncate_string(job.name, 30) if job.name else job.job_id[:20]
                    )
    ...
    def print_jobs_wide(
        ...
        for idx, job in enumerate(jobs, 1):
            ...
            if RICH_AVAILABLE:
                ...
                print(
                    f"[{idx}] {icon} {status_name} | {format_time_ago(job.created_at)} | {format_duration(job.running_time_ms)}"
                )
    ...
    def _print_jobs_plain(self, jobs: List[JobRecord]):
        ...
        for job in jobs:
            ...
            print(
                f"{job.job_id:<44} {truncate_string(job.name, 28):<30} {icon} {status_name:<10} {format_time_ago(job.created_at):<12}"
            )

        print("-" * 100)

    def print_job_detail(
        self, job: JobRecord, api_data: Optional[Dict[str, Any]] = None
    ):
        """打印任务详情"""
        if not RICH_AVAILABLE:
        ...
        if job.running_time_ms:
            info_lines.append(
                f"[bold]Duration:[/bold] {format_duration(job.running_time_ms)}"
            )
        ...
        if (
            job.gpu_count
            or job.instance_count
            or job.gpu_type
            or job.compute_group_name
        ):
            info_lines.append("")
